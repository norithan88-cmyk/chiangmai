...
function fetchJsonInsecure(url) {
  return new Promise((resolve, reject) => {
    const req = https.get(url, {
      headers: { 'user-agent': 'chiangmai-navi-air-collector/1.0 (+https://chiangmai.sunsun.live/)' },
...
const STATIONS = [
  { id: '35t', label: 'City Hall, Chiangmai（チェンマイ市役所）' },
  { id: '36t', label: 'Yupparaj Wittayalai School（ユッパラート・ウィッタヤーライ校）' },
];
...
async function main() {
  ...
  if (stations.length === 0) throw new Error('チェンマイの観測局データを1件も取得できなかったため、既存データを保持します。');
